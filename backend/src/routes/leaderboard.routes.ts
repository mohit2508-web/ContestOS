  ...
  try {
    ...
    const rankingsList = users.map((user) => {
      ...
      return {
        ...
        university: user.email.endsWith('.edu') || user.email.endsWith('.ac.in') ? 'Academic Partner' : 'Kryptavia OS University',
...
router.get('/global', async (req: Request, res: Response): Promise<void> => {
  ...
  const leaderboard = users.map((u, i) => ({
    ...
    university: 'Kryptavia OS University',
    department: 'Computer Science',
    trust_score: 92 - i,
...
router.get('/platform/:platform', async (req: Request, res: Response): Promise<void> => {
  ...
  const leaderboard = users.map((u, i) => ({
    ...
    university: 'Kryptavia OS Academy',
